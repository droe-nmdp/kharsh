...
refFasta = file("tutorial/ref/2DL4_2/2DL4.fasta")
refVCF = file("tutorial/ref/2DL4_2/2DL4-alleles_reads.txt")
refVariants = file("tutorial/ref/2DL4_2/2DL4-alleles_variants2.txt")//todo
refMatrix = file("tutorial/ref/2DL4_2/2DL4-alleles_matrix2_3c.txt")//todo
readNameSuffix = ".read"
raw = "${params.experiment}/raw/2DL4_2"
...
process trimFastq {
  ...
  output:
    set s, file{"${r1}_trimmed.fasta"}, file{"${r2}_trimmed.fasta"} into trimmedPairs

  """
  cat ${r1} > ${r1}_trimmed.fasta
  cat ${r2} > ${r2}_trimmed.fasta
  #todo head -n ${trimSize/2} ${r1} > ${r1}_trimmed.fasta
  #todo head -n ${trimSize/2} ${r2} > ${r2}_trimmed.fasta
  """
} // trimFastq
...
process fastq2BAM {
  ...
  output:
    set s, file {"${s}_2DL4_mapped.bam"} into personBAM

    """
  bwa mem -t2 ${refFasta} ${r1} ${r2} > ${s}_2DL4.sam 2> ${s}_2DL4_bwa.txt
  samtools view -b -S ${s}_2DL4.sam > ${s}_2DL4.bam
  samtools sort -o ${s}_2DL4_sorted.bam ${s}_2DL4.bam
  samtools view -b -F0x4 ${s}_2DL4_sorted.bam > ${s}_2DL4_mapped.bam
  samtools index ${s}_2DL4_mapped.bam
  #nice gzip -f ${s}_2DL4.sam
  #todo rm ${s}_2DL4.sam
  """    
} // fastq2BAM
